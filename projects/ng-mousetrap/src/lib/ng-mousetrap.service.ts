...
import { NgMousetrapOptions, INgMousetrapOptions } from './ng-mousetrap.options';
...
export class NgMousetrapService {
  ...
  constructor(@Inject(NgMousetrapOptions) private options: INgMousetrapOptions) {
    ...
    this.mousetrap.bind((<Hotkey>hotkey).combo, (event: KeyboardEvent, combo: string) => {
      ...
      if (event) {
        ...
        if ((' ' + target.className + ' ').indexOf(' mousetrap ') > -1) {
          shouldExecute = true;
        } else if (this._preventIn.indexOf(nodeName) > -1 && (<Hotkey>hotkey).allowIn
          .map(allow => allow.toUpperCase()).indexOf(nodeName) === -1) {
